...
import io.cdm.net.NIOHandler;
import io.cdm.net.NIOProcessor;
import io.cdm.net.mysql.AuthPacket;
import io.cdm.net.mysql.MySQLPacket;
import io.cdm.net.mysql.QuitPacket;
...
public class FrontendAuthenticator implements NIOHandler {
    ...
    public void handle(byte[] data) {
        ...
        //huangyiming add
        int nopassWordLogin = CDMServer.getInstance().getConfig().getSystem().getNonePasswordLogin();
        //如果无密码登陆则跳过密码验证这个步骤
        boolean skipPassWord = false;
        String defaultUser = "";
        if(nopassWordLogin == 1){
        	skipPassWord = true;
        	Map<String, UserConfig> userMaps =  CDMServer.getInstance().getConfig().getUsers();
        	if(!userMaps.isEmpty()){
    ...
    protected boolean isDegrade(String user) {
    	...
    	if ( benchmark > 0 ) {
	    	...
	    	NIOProcessor[] processors = CDMServer.getInstance().getProcessors();
