...
public class XMLSchemaLoader implements SchemaLoader {
     ...
     * 分片算法如下:<br>
     * {@code
     * <function name="mod-long" class="io.cdm.route.function.PartitionByMod">
     * <!-- how many data nodes -->
    ...
    private void loadDataNodes(Element root) {
        ...
        for (int i = 0, n = list.getLength(); i < n; i++) {
            ...
            String[] dnNames = io.cdm.util.SplitUtil.split(dnNamePre, ',', '$', '-');
            String[] databases = io.cdm.util.SplitUtil.split(databaseStr, ',', '$', '-');
            String[] hostStrings = io.cdm.util.SplitUtil.split(host, ',', '$', '-');
