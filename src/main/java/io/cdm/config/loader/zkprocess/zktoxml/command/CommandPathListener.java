package io.cdm.config.loader.zkprocess.zktoxml.command;
...
import io.cdm.config.loader.zkprocess.zktoxml.ZktoXmlMain;
...
public class CommandPathListener implements PathChildrenCacheListener {
    ...
    public void reload(final String path) {
        ...
        lock.lock();
        try {
            ListenableFuture<Boolean> listenableFuture = CDMServer.getInstance().getListeningExecutorService()
            ...
            }, CDMServer.getInstance().getListeningExecutorService());
        } finally {
            lock.unlock();
        }
    }
