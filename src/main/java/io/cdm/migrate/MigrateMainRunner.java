    ...
    public void run() {
        ...
        if (!forceBinlog) {
            ...
            for (MigrateTask migrateTask : migrateTaskList) {
                CDMServer.getInstance().getBusinessExecutor().submit(new MigrateDumpRunner(migrateTask, downLatch, sucessTask));
            }
            try {
        ...
        if (forceBinlog||(sucessTask.get() == migrateTaskList.size())) {
            ...
            //开始增量数据迁移
            PhysicalDBPool dbPool = CDMServer.getInstance().getConfig().getDataHosts().get(dataHost);
