                    ...
                    if ((!Strings.isNullOrEmpty(text)) && text.startsWith("{")) {
                        ...
                        if (taskNode.getStatus() == 0) {
                            ...
                            for (Map.Entry<String, List<MigrateTask>> stringListEntry : taskMap.entrySet()) {
                                ...
                                CDMServer.getInstance().getBusinessExecutor().submit(new MigrateMainRunner(key, value,taskNode.getTimeout(), Charset.forName(taskNode.getCharset()),taskNode.isForceBinlog()));
                            }
                            ...
                            ScheduledExecutorService scheduledExecutorService = CDMServer.getInstance().getScheduler();
