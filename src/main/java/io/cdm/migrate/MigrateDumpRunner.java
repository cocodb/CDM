...
import io.cdm.memory.environment.OperatingSystem;
import io.cdm.route.function.PartitionByCRC32PreSlot.Range;
import io.cdm.util.ProcessUtil;
import io.cdm.util.StringUtil;
import io.cdm.util.ZKUtils;
import io.cdm.util.dataMigrator.DataMigratorUtil;
...
import static io.cdm.util.dataMigrator.DataMigratorUtil.executeQuery;
...
public class MigrateDumpRunner implements Runnable {
    ...
    public void run() {
        try {
            ...
            PhysicalDBPool dbPool = CDMServer.getInstance().getConfig().getDataNodes().get(task.getFrom()).getDbPool();
    ...
    private void exeCreateTableToDn(String sql, String toDn, String table) throws SQLException {
        PhysicalDBNode dbNode = CDMServer.getInstance().getConfig().getDataNodes().get(toDn);
