...
import io.cdm.server.handler.MysqlInformationSchemaHandler;
import io.cdm.server.handler.MysqlProcHandler;
import io.cdm.server.parser.ServerParse;
import io.cdm.server.response.Heartbeat;
import io.cdm.server.response.InformationSchemaProfiling;
import io.cdm.server.response.Ping;
import io.cdm.server.util.SchemaUtil;
...
public class ServerConnection extends FrontendConnection {
	...
	public void execute(String sql, int type) {
		...
		SchemaConfig schema = CDMServer.getInstance().getConfig().getSchemas().get(db);
		if (schema == null) {
		...
		if (isDefault && schema.isCheckSQLSchema() && isNormalSql(type)) {
			...
			if (schemaInfo != null && schemaInfo.schema != null && !schemaInfo.schema.equals(db)) {
				SchemaConfig schemaConfig = CDMServer.getInstance().getConfig().getSchemas().get(schemaInfo.schema);
    ...
    public RouteResultset routeSQL(String sql, int type) {
		...
		SchemaConfig schema = CDMServer.getInstance().getConfig()
				.getSchemas().get(db);
		if (schema == null) {
			writeErrMessage(ErrorCode.ERR_BAD_LOGICDB,
					"Unknown MyCAT Database '" + db + "'");
