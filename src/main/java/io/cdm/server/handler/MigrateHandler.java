...
import io.cdm.migrate.MigrateTask;
import io.cdm.migrate.MigrateTaskWatch;
import io.cdm.migrate.MigrateUtils;
import io.cdm.migrate.TaskNode;
import io.cdm.net.mysql.*;
import io.cdm.route.function.AbstractPartitionAlgorithm;
import io.cdm.route.function.PartitionByCRC32PreSlot;
import io.cdm.route.function.PartitionByCRC32PreSlot.Range;
import io.cdm.server.ServerConnection;
import io.cdm.util.ObjectUtil;
...
import static io.cdm.config.loader.zkprocess.comm.ZkParamCfg.ZK_CFG_FLAG;
...
public final class MigrateHandler {
    ...
    public static void handle(String stmt, ServerConnection c) {
        ...
        try {
            ...
            SchemaConfig schemaConfig = CDMServer.getInstance().getConfig().getSchemas().get(schema);
            ...
            Map<String, PhysicalDBNode> allDataNodes = CDMServer.getInstance().getConfig().getDataNodes();
    ...
    private static String getDataHostNameFromNode(String dataNode) {
        return CDMServer.getInstance().getConfig().getDataNodes().get(dataNode).getDbPool().getHostName();
    }
    ...
    private static int getSlaveIdFromZKForDataNode(String dataNode) {
        PhysicalDBNode dbNode = CDMServer.getInstance().getConfig().getDataNodes().get(dataNode);
