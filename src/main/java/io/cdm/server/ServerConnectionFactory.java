...
import io.cdm.config.MycatPrivileges;
...
import io.cdm.server.handler.ServerLoadDataInfileHandler;
import io.cdm.server.handler.ServerPrepareHandler;
...
public class ServerConnectionFactory extends FrontendConnectionFactory {
    ...
    protected FrontendConnection getConnection(NetworkChannel channel) throws IOException {
        SystemConfig sys = CDMServer.getInstance().getConfig().getSystem();
        ServerConnection c = new ServerConnection(channel);
        CDMServer.getInstance().getConfig().setSocketParams(c, true);
