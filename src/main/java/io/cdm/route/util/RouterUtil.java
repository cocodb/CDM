...
import io.cdm.backend.mysql.nio.handler.FetchStoreNodeOfChildTableHandler;
...
import io.cdm.route.SessionSQLPair;
...
import io.cdm.sqlengine.mpp.ColumnRoutePair;
...
//		int sequenceHandlerType = CDMServer.getInstance().getConfig().getSystem().getSequnceHandlerType();
...
//		CDMServer.getInstance().getSequnceProcessor().addNewSql(sessionSQLPair);
        CDMServer.getInstance().getSequenceExecutor().execute(new Runnable() {
				@Override
				public void run() {
					CDMServer.getInstance().getSequnceProcessor().executeSeq(sessionSQLPair);
				}
		 });
...
//    }
	...
	private static String getAliveRandomDataNode(TableConfig tc) {
		...
		MycatConfig mycatConfig = CDMServer.getInstance().getConfig();
		if (mycatConfig != null) {
			...
			for (String randomDn : randomDns) {
				...
				if (physicalDBNode != null) {
					if (physicalDBNode.getDbPool().getSource().isAlive()) {
						for (PhysicalDBPool pool : CDMServer.getInstance().getConfig().getDataHosts().values()) {
    ...
    private static String getRandomDataNode(TableConfig tc) {
        ...
        if (mycatConfig != null) {
            ...
            if (physicalDBNode != null) {
                if (physicalDBNode.getDbPool().getSource().isAlive()) {
                    for (PhysicalDBPool pool : CDMServer.getInstance()
                            .getConfig()
                            .getDataHosts()
                            .values()) {
...
//				MySqlInsertStatement insertStmt = (MySqlInsertStatement) (new MySqlStatementParser(origSQL)).parseInsert();
				...
				ListenableFuture<String> listenableFuture = CDMServer.getInstance().
