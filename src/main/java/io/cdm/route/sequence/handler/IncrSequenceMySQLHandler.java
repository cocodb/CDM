package io.cdm.route.sequence.handler;
...
import io.cdm.route.util.PropertiesUtil;
...
//		newValueSetted.set(false);
	...
	public Long[] waitFinish(FetchMySQLSequnceHandler mysqlSeqFetcher, int retryCount, boolean canSendFetch) {
		final int systemRetryCount = CDMServer.getInstance().getConfig().getSystem().getSequnceMySqlRetryCount();
		...
		//直接等待
		long mysqlWaitTime = CDMServer.getInstance().getConfig().getSystem().getSequnceMySqlWaitTime();
