...
import io.cdm.backend.mysql.nio.handler.MiddlerQueryResultHandler;
import io.cdm.backend.mysql.nio.handler.MiddlerResultHandler;
import io.cdm.backend.mysql.nio.handler.SecondHandler;
...
import io.cdm.route.impl.middlerResultStrategy.BinaryOpResultHandler;
import io.cdm.route.impl.middlerResultStrategy.InSubQueryResultHandler;
import io.cdm.route.impl.middlerResultStrategy.RouteMiddlerReaultHandler;
import io.cdm.route.impl.middlerResultStrategy.SQLAllResultHandler;
import io.cdm.route.impl.middlerResultStrategy.SQLExistsResultHandler;
import io.cdm.route.impl.middlerResultStrategy.SQLQueryResultHandler;
import io.cdm.route.parser.druid.DruidParser;
import io.cdm.route.parser.druid.DruidParserFactory;
import io.cdm.route.parser.druid.DruidShardingParseInfo;
import io.cdm.route.parser.druid.MycatSchemaStatVisitor;
import io.cdm.route.parser.druid.MycatStatementParser;
import io.cdm.route.parser.druid.RouteCalculateUnit;
...
public class DruidMycatRouteStrategy extends AbstractRouteStrategy {
	...
	public RouteResultset routeNormalSqlWithAST(SchemaConfig schema,
		...
		SchemaConfig schemaConf = CDMServer.getInstance().getConfig().getSchemas().get(schema.getName());
		int index = 0;
	...
	private boolean checkRuleField(Map<String,RuleConfig> rulemap,MycatSchemaStatVisitor visitor){
		...
		if(!CDMServer.getInstance().getConfig().getSystem().isSubqueryRelationshipCheck()){
			return true;
		}
				...
				public void doExecute(List param) {
					...
					try {
						...
						rrs = CDMServer
								.getInstance()
								.getRouterservice()
								.route(CDMServer.getInstance().getConfig().getSystem(),
	...
	private RouteResultset catletRoute(SchemaConfig schema,String stmt,String charset,ServerConnection sc){
		...
		try {
			rrs = CDMServer
					...
					.getRouterservice()
					.route(CDMServer.getInstance().getConfig().getSystem(),
							schema, ServerParse.SELECT, "/*!mycat:catlet=io.cdm.catlets.ShareJoin */ "+stmt, charset, sc);
