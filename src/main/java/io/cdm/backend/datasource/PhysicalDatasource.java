...
import io.cdm.backend.ConMap;
import io.cdm.backend.ConQueue;
import io.cdm.backend.heartbeat.DBHeartbeat;
import io.cdm.backend.mysql.nio.MySQLConnection;
import io.cdm.backend.mysql.nio.handler.ConnectionHeartBeatHandler;
import io.cdm.backend.mysql.nio.handler.DelegateResponseHandler;
import io.cdm.backend.mysql.nio.handler.NewConnectionRespHandler;
...
//		}
	...
	private void createNewConnection(final ResponseHandler handler,
		...
		CDMServer.getInstance().getBusinessExecutor().execute(new Runnable() {
			public void run() {
				try {
