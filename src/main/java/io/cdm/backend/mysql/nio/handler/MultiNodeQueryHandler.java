 ...
 *
 */
package io.cdm.backend.mysql.nio.handler;
...
import io.cdm.sqlengine.mpp.AbstractDataNodeMerge;
import io.cdm.sqlengine.mpp.ColMeta;
import io.cdm.sqlengine.mpp.DataMergeService;
import io.cdm.sqlengine.mpp.DataNodeMergeManager;
import io.cdm.sqlengine.mpp.MergeCol;
import io.cdm.statistic.stat.QueryResult;
import io.cdm.statistic.stat.QueryResultDispatcher;
...
public class MultiNodeQueryHandler extends MultiNodeHandler implements LoadDataResponseHandler {
	...
	public MultiNodeQueryHandler(int sqlType, RouteResultset rrs,
		...
		this.rrs = rrs;
		isOffHeapuseOffHeapForMerge = CDMServer.getInstance().
	...
	public void rowEofResponse(final byte[] eof, BackendConnection conn) {
		...
		if(middlerResultHandler !=null){
 		...
 		if (execCount == rrs.getNodes().length) {
			int resultSize = source.getWriteQueue().size()* CDMServer.getInstance().getConfig().getSystem().getBufferPoolPageSize();
			source.setExecuteSql(null);  //完善show @@connection.sql 监控命令.已经执行完的sql 不再显示
			//TODO: add by zhuam
			//查询结果派发
			QueryResult queryResult = new QueryResult(session.getSource().getUser(),
		...
		try {
			...
			if (dataMergeSvr != null) {
				...
				if (primaryKeyIndex != -1) {
					...
					LayerCachePool pool = CDMServer.getInstance().getRouterservice().getTableId2DataNodeCache();
