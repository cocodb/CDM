package io.cdm.backend.heartbeat.zkprocess;
...
public class ManageHeartBeatChange implements Runnable {
	...
	final ScheduledExecutorService service = CDMServer.getInstance().getHeartbeatScheduler(); //定时器
	...
	public void run() {
		...
		try {
			...
			if(maxIndex != -1) { 
				...
				boolean result = CDMServer.getInstance().saveDataHostIndexToZk(dataHost, maxIndex);
				if(result) {
					//开启对结果切换的监控
