...
import io.cdm.backend.postgresql.PostgreSQLBackendConnection.BackendConnectionState;
import io.cdm.backend.postgresql.packet.AuthenticationPacket;
import io.cdm.backend.postgresql.packet.AuthenticationPacket.AuthType;
import io.cdm.backend.postgresql.packet.BackendKeyData;
import io.cdm.backend.postgresql.packet.CommandComplete;
import io.cdm.backend.postgresql.packet.CopyInResponse;
import io.cdm.backend.postgresql.packet.CopyOutResponse;
import io.cdm.backend.postgresql.packet.DataRow;
import io.cdm.backend.postgresql.packet.EmptyQueryResponse;
import io.cdm.backend.postgresql.packet.ErrorResponse;
import io.cdm.backend.postgresql.packet.NoticeResponse;
import io.cdm.backend.postgresql.packet.NotificationResponse;
import io.cdm.backend.postgresql.packet.ParameterStatus;
import io.cdm.backend.postgresql.packet.PasswordMessage;
import io.cdm.backend.postgresql.packet.PostgreSQLPacket;
import io.cdm.backend.postgresql.packet.ReadyForQuery;
import io.cdm.backend.postgresql.packet.ReadyForQuery.TransactionState;
import io.cdm.backend.postgresql.packet.RowDescription;
import io.cdm.backend.postgresql.utils.PacketUtils;
import io.cdm.backend.postgresql.utils.PgPacketApaterUtils;
import io.cdm.buffer.BufferArray;
import io.cdm.config.ErrorCode;
import io.cdm.net.handler.BackendAsyncHandler;
...
public class PostgreSQLBackendConnectionHandler extends BackendAsyncHandler {
	...
	private void doProcessBusinessQuery(PostgreSQLBackendConnection con,
		...
		BufferArray bufferArray = CDMServer.getInstance().getBufferPool()
				.allocateArray();
		...
		while (itor.hasNext()) {
			bufferArray = CDMServer.getInstance().getBufferPool()
					.allocateArray();
		...
		bufferArray = CDMServer.getInstance().getBufferPool().allocateArray();
	 ...
	 * 
	 * 
	 * @see io.cdm.net.handler.BackendAsyncHandler#handleData(byte[])
	 */
	@Override
