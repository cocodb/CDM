...
import io.cdm.statistic.SQLRecord;
...
//	private ReentrantReadWriteLock lock  = new ReentrantReadWriteLock();
	...
	public UserStat(String user) {
		...
		int size = CDMServer.getInstance().getConfig().getSystem().getSqlRecordCount();
...
//        try {
			...
			//记录超过阈值的大结果集sql
			if(rseultSetSize>= CDMServer.getInstance().getConfig().getSystem().getMaxResultSet()){
