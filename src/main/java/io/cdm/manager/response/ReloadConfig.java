...
import io.cdm.config.ConfigInitializer;
...
import io.cdm.config.util.DnPropertyUtil;
...
public final class ReloadConfig {
	...
	public static void execute(ManagerConnection c, final boolean loadAll) {
		...
		final ReentrantLock lock = CDMServer.getInstance().getConfig().getLock();
		lock.lock();
		try {
			ListenableFuture<Boolean> listenableFuture = CDMServer.getInstance().getListeningExecutorService().submit(
			...
			Futures.addCallback(listenableFuture, new ReloadCallBack(c), CDMServer.getInstance().getListeningExecutorService());
		} finally {
			lock.unlock();
		}
	}
	...
	public static boolean reload_all() {
		...
		MycatConfig config = CDMServer.getInstance().getConfig();
		...
		if ( isReloadStatusOK ) {
			...
			for (PhysicalDBPool dbPool : oldDataHosts.values()) {			
				...
				for (PhysicalDatasource ds : dbPool.getAllDataSources()) {					
					//
					for (NIOProcessor processor : CDMServer.getInstance().getProcessors()) {
			...
			CDMServer.getInstance().initRuleData();
			return true;
    ...
    public static boolean reload() {
         ...
         * 2、在老的配置上，应用新的配置
         */
        CDMServer.getInstance().getConfig().reload(users, schemas, dataNodes, dataHosts, cluster, firewall, false);
