...
import io.cdm.route.parser.ManagerParseSwitch;
import io.cdm.route.parser.util.Pair;
...
public final class SwitchDataSource {
    ...
    public static void response(String stmt, ManagerConnection c) {
        ...
        Map<String, PhysicalDBPool> dns = CDMServer.getInstance().getConfig().getDataHosts();
            ...
            if (dn != null) {
                ...
                if(!dn.notSwitchSource(n)) {
                    ...
                    if(CDMServer.getInstance().isUseZkSwitch()) {
                    	CDMServer.getInstance().saveDataHostIndexToZk(dn.getHostName(), n);
                        ++count;
