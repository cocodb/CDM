...
public final class ConfFileHandler {
	...
	private static void checkXMLFile(String xmlFileName, byte[] data)
		...
		if (xmlFileName.equals("schema.xml")) {
			dtdStream = CDMServer.class.getResourceAsStream("/schema.dtd");
			if (dtdStream == null) {
			...
			dtdStream = CDMServer.class.getResourceAsStream("/server.dtd");
			if (dtdStream == null) {
				dtdStream = new ByteArrayInputStream(readFileByBytes(new File(
						confDir, "server.dtd")));
			}
		} else if (xmlFileName.equals("rule.xml")) {
			dtdStream = CDMServer.class.getResourceAsStream("/rule.dtd");
			if (dtdStream == null) {
				dtdStream = new ByteArrayInputStream(readFileByBytes(new File(
						confDir, "rule.dtd")));
			}
